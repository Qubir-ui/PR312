...
class SecondActivityTest{
    ...
    private val text = "text_from_second"
    @Test
    fun accordanceText(){
        onView(withId(R.id.button_main)).perform(click())
        onView(withId(R.id.editText_second)).perform(ViewActions.typeText(text))
        onView(withId(R.id.button_second)).perform(click())
        onView(withId(R.id.text_message_reply)).check(matches(withText("text_from_second")))
    }
}